...
public class Combo extends Buff implements ActionIndicator.Action {
	...
	public void hit( Char enemy ) {
		...
		if (!enemy.isAlive() || (enemy.buff(Corruption.class) != null && enemy.HP == enemy.HT)){
			comboTime = Math.max(comboTime, 33*((Hero)target).pointsInTalent(Talent.CLEAVE, Talent.ENDURANCE));
			if (((Hero)target).pointsInTalent(Talent.CLEAVE, Talent.ENDURANCE) >= 3) comboTime = Integer.MAX_VALUE;
		}
		...
		if (hero.attack(enemy, dmgMulti, dmgBonus, Char.INFINITE_ACCURACY)){
			...
			switch (moveBeingUsed) {
				case CLOBBER:
					...
					int dist = 2;
					if (enemy.isAlive() && count >= 4 && hero.pointsInTalent(Talent.ENHANCED_COMBO, Talent.DIVERSITY) >= 1) {
						dist*= 3;
				...
				case CRUSH:
					...
					for (Char ch : Actor.chars()) {
						if (ch != enemy && ch.alignment == Char.Alignment.ENEMY
							...
							if (!ch.isAlive()) {
								if (hero.hasTalent(Talent.LETHAL_DEFENSE, Talent.REACTION) && hero.buff(BrokenSeal.WarriorShield.class) != null) {
									BrokenSeal.WarriorShield shield = hero.buff(BrokenSeal.WarriorShield.class);
									shield.supercharge(Math.round(shield.maxShield() * hero.pointsInTalent(Talent.LETHAL_DEFENSE, Talent.REACTION)*1.33f));
								}
							}
						}
					}
	...
	private CellSelector.Listener listener = new CellSelector.Listener() {
		...
		public void onSelect(Integer cell) {
			...
			if (enemy == null
				...
				if (((Hero) target).pointsInTalent(Talent.ENHANCED_COMBO, Talent.DIVERSITY) < 1
