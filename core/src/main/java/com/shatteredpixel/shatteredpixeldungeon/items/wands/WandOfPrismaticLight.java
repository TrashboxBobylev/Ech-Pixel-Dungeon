...
public class WandOfPrismaticLight extends DamageWand {
	...
	public void onZap(Ballistica beam) {
		...
		if (Dungeon.level.viewDistance < 6 ){
			if (Dungeon.isChallenged(Challenges.DARKNESS)){
				Buff.prolong( curUser, Light.class, 5f + buffedLvl());
			} else {
				Buff.prolong( curUser, Light.class, 20f+buffedLvl()*11);
			}
		}
	...
	private void affectTarget(Char ch){
		...
		if (Random.Int(5+buffedLvl()) >= 3) {
			Buff.prolong(ch, Blindness.class, 5f + (buffedLvl()));
		...
		if (ch.properties().contains(Char.Property.DEMONIC) || ch.properties().contains(Char.Property.UNDEAD)){
			...
			ch.damage(Math.round(dmg*1.75f), this);
		} else {
